def mash_line(arg, arg1):
    ...
    #5) удаляем столбцы, где нет данных(стоят нули во всей колонке), j range (0,10), для индекса - 10 строк(2010-2020)
    for j in range(0,10):
...
#12) Формула выглядит так - y = kx + b, т.е прямая линия, если коэфициент положительный, тогда он идет наверх, если = 0, тогда прямо, если отрицательный то вниз.

# reg.coef_ - можно посмотреть коэфициент. df2.corr() корреляция (зависимости величин друг от друга, чем меньше, меньше влияет)
